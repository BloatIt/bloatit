...
public class LanguageField extends LanguageSelector {

    public LanguageField(final UserContentActionUrl targetUrl, final String label, final String comment) {
        super(targetUrl.getLocaleParameter().pickFieldData().getName(), label);
        final FieldData localeData = targetUrl.getLocaleParameter().pickFieldData();
        setDefaultValue(localeData.getSuggestedValue(), Context.getLocalizator().getLanguageCode());
        addErrorMessages(localeData.getErrorMessages());
        setComment(comment);
    }
