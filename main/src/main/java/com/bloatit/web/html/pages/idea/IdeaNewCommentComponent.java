...
import com.bloatit.web.actions.NewCommentAction;
...
public class IdeaNewCommentComponent extends HtmlPageComponent {
    ...
    protected HtmlElement produce() {
    	final NewCommentActionUrl url = new NewCommentActionUrl(targetIdea);
        final HtmlDiv commentBlock = new HtmlDiv("new_comment_block");
        
        HtmlForm form = new HtmlForm(url.urlString());
        commentBlock.add(form);
        
        HtmlTextArea commentInput = new HtmlTextArea(NewCommentAction.COMMENT_CONTENT_CODE, Context.tr("Comment : "), 10, 80);
