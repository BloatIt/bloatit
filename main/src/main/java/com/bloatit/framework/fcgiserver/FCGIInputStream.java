...
public class FCGIInputStream extends InputStream {
     ...
     * 
     * @param in
     *            the input stream bufLen length of buffer streamType
     */
    ...
    public void fill() throws IOException {
        byte[] headerBuf = new byte[FCGIGlobalDefs.FCGI_HEADER_LEN];
        int headerLen = 0;
        int status = 0;
        int count = 0;
        for (;;) {
            ...
            if (rdNext == buffStop) {
                ...
                if (count == 0) {
                    setFCGIError(FCGIGlobalDefs.FCGI_PROTOCOL_ERROR);
                    return;
                }
                rdNext = 0;
            ...
            // Interpret the header. eorStop prevents ProcessHeader from
            ...
            isClosed = false;
            switch (status) {
            case FCGIGlobalDefs.FCGI_STREAM_RECORD:
                if (contentLen == 0) {
                    stop = rdNext;
                    isClosed = true;
                    return;
                ...
                break;
            case FCGIGlobalDefs.FCGI_SKIP:
                skip = true;
                break;
            case FCGIGlobalDefs.FCGI_BEGIN_RECORD:
                // If this header marked the beginning of a new request, return
                // role info to caller
                return;
            case FCGIGlobalDefs.FCGI_MGMT_RECORD:
                break;
            default:
                // ASSERT
                setFCGIError(status);
     ...
     * </p>
     * <p>
     * This method does not really exist for BufferedInputStream in java, but is
     * implemented here for compatibility with the FCGI structures being used.
     * It doent really throw any IOExceptions either, but that's there for
     * compatiblity with the InputStreamInterface.
     * </p>
     */
