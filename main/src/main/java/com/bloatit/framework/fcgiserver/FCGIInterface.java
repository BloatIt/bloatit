...
/**
 * <p>
 * This is the FastCGI interface that the application calls to communicate with
 * the FastCGI web server. This version is single threaded, and handles one
 * request at a time, which is why we can have a static variable for it.
 * </p>
 */
public class FCGIInterface {
    ...
    int FCGIAccept() throws IOException {
        ...
        for (;;) {
            ...
            request.inStream = new FCGIInputStream((FileInputStream) request.socket.getInputStream(), 8192, 0, request);
        ...
        switch (request.role) {
        case FCGIGlobalDefs.FCGI_RESPONDER:
            request.params.put("ROLE", "RESPONDER");
            break;
        case FCGIGlobalDefs.FCGI_AUTHORIZER:
            request.params.put("ROLE", "AUTHORIZER");
            break;
        case FCGIGlobalDefs.FCGI_FILTER:
            request.params.put("ROLE", "FILTER");
            break;
        default:
            return -1;
        }
        request.inStream.setReaderType(FCGIGlobalDefs.FCGI_PARAMS);
        /*
        ...
        request.inStream.setReaderType(FCGIGlobalDefs.FCGI_STDIN);
        request.outStream = new FCGIOutputStream((FileOutputStream) request.socket.getOutputStream(), 8192, FCGIGlobalDefs.FCGI_STDOUT, request);
        request.errStream = new FCGIOutputStream((FileOutputStream) request.socket.getOutputStream(), 512, FCGIGlobalDefs.FCGI_STDERR, request);
