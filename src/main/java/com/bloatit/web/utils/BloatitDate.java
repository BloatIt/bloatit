 ...
 * <p>Encapsulates all operations concerning date parsing</p>
 * <p>Date format is represented by 2 elements :
 * * The locale that gives the order of the elements (example : fr : dd/mm/yyyy)
 * * The style that determines the length of the date representation</p>
 *
 * <p>BloatidDate can be used to parse string into dates (usually after a user
 * wrote a date in a form) or to find the representation of a date for a given
 * locale (usually to display a date stored in the database)</p>
 *
 * <p>When parsing a string, Parser will <b>NOT</b> use heuristics meaning the
 * input has to match the pattern, or an exception will be thrown. This
 * comportment allows much better error detections</p>
 * <p>For obvious reasons, parser will not be able to detect errors involving
 * inversion of days and month when day is inferior to 13</p>
 * <p>Note that, when constructing a BloatitDate from a String, no proactive
...
public final class BloatitDate {
    ...
    /**
     * <p>Describes the format of the date</p>
     * <p>
    ...
    /**
     * <p>Creates a BloatItDate based on a string and a locale</p>
     * <p>The date must be short format, aka 31/12/2010</p>
     ...
     */
    public BloatitDate(String dateString, Locale locale) throws DateParsingException {
     ...
     * that matches the current user locale
     */
    public BloatitDate(String dateString, Locale locale, FormatStyle style) throws DateParsingException {
    ...
    /**
     * <p>Creates a BloatItDate based on a java Date with a defaut SHORT
     * style</p>
     * @param javaDate the Date to convert
     * @param locale the locale for the date
     */
    public BloatitDate(Date javaDate, Locale locale){
        this(javaDate, locale, FormatStyle.SHORT);
    }

    /**
     * <p>Creates a BloatItDate based on a java Date</p>
     * @param javaDate the Date to convert
     * @param locale the locale for the date
     * @param style the style of the date
     */
    public BloatitDate(Date javaDate, Locale locale, FormatStyle style){
        this.locale = locale;
    ...
    /**
     * <p>Modifies the date.</p>
     * <p>This method is provided for efficiency only. Clean usage should
     * involve creating a new object for every dates</p>
     * @param dateString the String representing the new date value
     * @throws DateParsingException When dateString doesn't contain a String
     * that matches the current user locale
     */
    public void setDate(String dateString) throws DateParsingException{
        try{
            this.dateString = dateString;
            this.parseDate();
    ...
    public String toString(FormatStyle style){
        DateFormat df = DateFormat.getDateInstance(this.getJavaStyle(style), this.locale);
    ...
    /**
     * <p>Converts this.style into a style usable by the Java DateFormat
     * Object</p>
    ...
    /**
     * <p>Converts a given style into a style usable by the Java
     * DateFormat Object</p>
     * @param style the style to convert
     * @return the converted style
     */
    private static int getJavaStyle(FormatStyle style){
        switch (style) {
            case MEDIUM:
     ...
     * @return
     */
    static public String getPattern(Locale locale){
     ...
     * @return
     */
    static public String getPattern(Locale locale, FormatStyle format){
        SimpleDateFormat sdf = (SimpleDateFormat)DateFormat.getDateInstance(getJavaStyle(format), locale);
        return cleanPattern(sdf.toPattern());
    }

    /**
     * <p>Make the pattern shiny for user interaction</p>
     * <p>Make sure the pattern is ready to be displayed to the user :
     * * Days and months will be shown with 2 digits
     * * Year will be shown with 4 digits
     * * Remove all caps
     * </p>
     * @param pattern the ugly pattern
     * @return the nice pattern
     */
    static private String cleanPattern(String pattern){
        String separator = "";
        ...
        String[] dateElements = pattern.toLowerCase().split(separator);
        StringBuilder outDate = new StringBuilder();
        
        for(String element : dateElements){
            if(outDate.length() > 0){
